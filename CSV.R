...
DOJ<-vector(mode='integer',length=n)
empcode<-vector(mode='integer',length=n)
dept<-vector(mode='character',length=n)
design<-vector(mode='character',length=n)

for(i in 1:n) {
  print(paste('enter the employees empid ename DOJ empcode dept and design : ', i))
  empid[i]<-as.integer(readline())
  ename[i]<-as.character(readline())
  DOJ[i]<-as.integer(readline())
  empcode[i]<-as.integer(readline())
  dept[i]<-as.character(readline())
  design[i]<-as.character(readline())
}
emp<-data.frame(empid,ename,DOJ,dept,design)
print(emp)
write.csv(emp, file="~/Desktop/emp.csv",row.names = FALSE)
print(read.csv(("~/Desktop/emp.csv"))
